...
class ImprovedAgent(Player):
    def __init__(self):
        ...
        self.engine = chess.engine.SimpleEngine.popen_uci(stockfish_path, timeout=70)
    ...
    def handle_sense_result(self, sense_result: List[Tuple[Square, Optional[chess.Piece]]]):
        ...
        # Initialize a set to store consistent states
        consistent_states = set()

        # Add the current board configuration as a consistent state
        consistent_states.add(self.board.fen())

        # Iterate over each state in the possible states
        for state in self.possible_states:
            board = chess.Board(state)
            valid = True

            # Check if the sense result matches the state
            for square, piece in sense_result:
                if board.piece_at(square) != piece:
                    valid = False
                    break

            if valid and board.is_valid():
                consistent_states.add(state)

            # If placing the detected piece results in an invalid board, create new boards by removing conflicting pieces
            else:
                for square, piece in sense_result:
                    if piece is not None:
                        # If the detected piece conflicts with an existing piece on the board, remove that conflicting piece and add the resulting board to consistent states
                        if board.piece_at(square) is not None:
                            new_board = board.copy()
                            new_board.remove_piece_at(square)
                            if new_board.is_valid():
                                consistent_states.add(new_board.fen())
